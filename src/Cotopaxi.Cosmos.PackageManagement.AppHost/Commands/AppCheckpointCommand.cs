...
internal sealed class AppCheckpointCommand : Command
{
    public readonly Argument<string> PackageArgument =
        new("package", "The path to the database package or packages for deployment to the Azure Cosmos DB account");
    public readonly Argument<string> RollbackPackageArgument =
        new("rollback-package", "The path to a resulting database package with rollback operations");
    ...
    {
        RollbackPackageArgument.AddValidationAsOutputFile();
        EndpointOption.AddValidationAsHttpsUri();

        AddArgument(PackageArgument);
        AddArgument(RollbackPackageArgument);
